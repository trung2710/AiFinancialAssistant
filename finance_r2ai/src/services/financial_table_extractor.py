...
class FinancialTableExtractor:
    ...
    def _is_noise_text(self, text_chunk: str) -> int:
        ...
        for line in lines:
            ...
            if "công ty" in line_lower and (
                    "cổ phần" in line_lower or "trách nhiệm" in line_lower or "tnhh" in line_lower):
                continue
    ...
    def _extract_table_name_and_unit(self, context_text: str):
        ...
        for line in reversed(lines):
            ...
            if "đơn vị tính" in line_lower or "đvt" in line_lower:
                import re
                match = re.search(
                    r'(?i)(?:đơn vị tính|đvt)\s*[:\-\s]\s*(.+)', line)
                if match:
        ...
        for line in reversed(lines):
            ...
            if line.isdigit():
                continue
            if "công ty" in line_lower and (
                    "cổ phần" in line_lower or "tnhh" in line_lower):
                continue
    ...
    def process(self, raw_txt: str, folder_name: str = "") -> tuple:
        # Cắt kẹp chả trực tiếp từ raw_txt để bảo toàn 100% văn bản gốc (kể cả
        # các thẻ ===== PAGE X =====)
        parts = re.split(r'(<table.*?</table>)', raw_txt,
        ...
        for i in range(0, len(parts) - 1, 2):
            ...
            # Xóa tạm marker ngắt trang cho việc tính toán noise_text và trích
            # metadata
            clean_context_text = re.sub(
            ...
            table_name, unit = self._extract_table_name_and_unit(
                clean_context_text)
            ...
            if pending_chunk is not None:
                is_repeated = self._is_repeated_header(
                    pending_chunk['dataframe'], df_current)
                # Tín hiệu gộp bảng: có chữ "tiếp theo" VÀ đi kèm tên loại
                # bảng/báo cáo
                context_lower = clean_context_text.lower()
                has_tiep_theo = "tiếp theo" in context_lower and any(
                    kw in context_lower for kw in ["bảng", "báo cáo", "thuyết minh"])

                # Nếu tiêu đề trùng khớp hoàn toàn HOẶC có chữ "tiếp theo"
                allowed_noise = 10 if (
                    is_repeated or has_tiep_theo) else self.noise_threshold

                if (self._is_noise_text(clean_context_text) <= allowed_noise and len(
                        df_current.columns) == len(pending_chunk['dataframe'].columns)):
                    ...
                    merged_df = pd.concat(
                        [pending_chunk['dataframe'], df_current], ignore_index=True)

                    # Cập nhật DataFrame và nối thêm mã HTML gốc của bảng nối
                    # trang
                    ...
                    pending_chunk['dataframe'] = self._normalize_numeric_df(
                        pending_chunk['dataframe'])
...
#     with open("/home/newuser/Code/AiFinancialAssistant/instructions/data/financial_statements/ABB/2022/ABB_financial_statements_2022_consolidated/ABB_financial_statements_2022_consolidated_extracted.txt", "r", encoding="utf-8") as file:
#         raw_text = file.read()
...
#     # 2. Gọi hàm xử lý
#     chunks, sync_text = extractor.process(raw_text)

#     # 3. In kết quả
#     for i, chunk in enumerate(chunks):
#         print(f"\n--- DATAFRAME THỨ {i+1} ---")
#         print(chunk['dataframe'].to_string())
